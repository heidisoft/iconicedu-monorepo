...
import { View, Text, ScrollView, TouchableOpacity, Pressable, StyleSheet, RefreshControl } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { useRouter } from 'expo-router';
import { Bell, CheckCircle, ClipboardCheck, CreditCard, FileText, GraduationCap, MessageCircle, MessageSquare, Paperclip, Sparkles, User, Video } from 'lucide-react-native';
import { useAuth } from '@/providers/auth-provider';
import { useProfile } from '@/hooks/use-profile';
import { useTheme } from '@/providers/theme-provider';
import type { AppColors } from '@/lib/theme';
import type { ActivityFeedItemVM, ActivityFeedGroupItemVM, ActivityFeedLeafItemVM, InboxIconKeyVM } from '@iconicedu/shared-types';
...
const ICON_MAP: Record<InboxIconKeyVM, React.ComponentType<{ size: number; color: string }>> = {
  Bell, CheckCircle2: CheckCircle, ClipboardCheck, CreditCard,
  FileText, GraduationCap, MessageSquare, Paperclip, Sparkles, Video,
};
...
function getRecentItems(n: number): ActivityFeedItemVM[] {
  return DEMO_ACTIVITY_FEED.sections
    .flatMap(s => s.items)
    .filter(item => item.tabKey === 'classes')
    .sort((a, b) => new Date(b.timestamps.occurredAt).getTime() - new Date(a.timestamps.occurredAt).getTime())
    .slice(0, n);
}
  ...
  return (
    <View style={s.itemOuter}>
      ...
      >
        ...
        {hasExpandedContent && (
          <TouchableOpacity onPress={() => onToggle(item.ids.id)} hitSlop={8} style={s.readMoreBtn}>
...
export default function HomeScreen() {
  ...
  const profileData = profile as { first_name?: string | null; display_name?: string | null; avatar_seed?: string | null; ui_theme_key?: string | null } | null;
  const firstName = profileData?.first_name?.trim() || profileData?.display_name?.trim()?.split(' ')[0] || user?.email?.split('@')[0] || 'there';
  const initial = firstName[0]?.toUpperCase() ?? 'U';
  const { bg: avatarBg, fg: avatarFg } = resolveAvatarColor(profileData?.ui_theme_key, profileData?.avatar_seed ?? user?.id ?? user?.email);

  const recentItems = React.useMemo(() => getRecentItems(5), []);
    ...
    <SafeAreaView style={s.safe}>
      ...
      >
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <TouchableOpacity style={[s.avatar, { backgroundColor: avatarBg }]} onPress={() => router.push('/(app)/(tabs)/account')} activeOpacity={0.8} accessibilityLabel="Open account">
        ...
        {/* Greeting */}
        <View style={{ gap: 6 }}>
          <Text style={s.greetingLine}>{getGreeting()}, {firstName} 👋</Text>
        ...
        {recentItems.length > 0 && (
          <View style={{ gap: 10 }}>
            <View style={s.activityHeader}>
              ...
              <TouchableOpacity onPress={() => router.push('/(app)/(tabs)/inbox')} hitSlop={8}>
