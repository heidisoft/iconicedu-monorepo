...
import Svg, { Path, Line, Defs, LinearGradient as SvgLinearGradient, Stop, Mask, Rect, G } from 'react-native-svg';
  ...
  return (
    <View pointerEvents="none" style={StyleSheet.absoluteFill}>
        ...
        <Svg width={gW} height={gH}>
          ...
          <G mask="url(#gridMask)">
            {Array.from({ length: GRID_COLS + 1 }).map((_, i) => (
              <Line key={`v${i}`} x1={i * GRID_CELL} y1={0} x2={i * GRID_CELL} y2={gH} stroke={grid} strokeWidth={1} />
            ))}
            {Array.from({ length: GRID_ROWS + 1 }).map((_, i) => (
              <Line key={`h${i}`} x1={0} y1={i * GRID_CELL} x2={gW} y2={i * GRID_CELL} stroke={grid} strokeWidth={1} />
            ))}
          </G>
        </Svg>
      </View>
      ...
      <View style={{
        ...
        borderWidth: 20, borderColor: ring,
      }} />

      {/* Circles cluster — top left, partially off-screen */}
      <View style={{ position: 'absolute', top: 52, left: -20, gap: 8, flexDirection: 'row', flexWrap: 'wrap', width: 116 }}>
        {([
          { bg: teal1 },
          { bg: teal2 },
          { bg: teal3 },
        ...
        ] as { bg: string; bTL?: number; bBR?: number }[]).map((c, i) => (
          <View
            key={i}
            style={{
      ...
      <View style={{
        position: 'absolute', bottom: 100, right: 28,
        width: 68, height: 68,
        transform: [{ rotate: '45deg' }],
        borderWidth: 14, borderColor: sq,
      }} />
...
function makeStyles(C: AppColors) {
  return StyleSheet.create({
    ...
    heading:  { fontSize: 28, fontWeight: '700', color: C.text, lineHeight: 36, letterSpacing: -0.3, textAlign: 'center' },
    ...
    cta:      { backgroundColor: C.teal, borderRadius: 14, paddingVertical: 16, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8 },
    ctaDim:   { opacity: 0.6 },
...
export default function LoginScreen() {
  ...
  const handleGoogle = useCallback(async () => {
    ...
    if (googleErr) setGoogleError(googleErr);
