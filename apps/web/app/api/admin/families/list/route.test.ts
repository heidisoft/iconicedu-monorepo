...
import { GET } from '@iconicedu/web/app/api/admin/families/list/route';
import { resolveAppUrl } from '@iconicedu/web/lib/config/app-url';

const createSupabaseServerClient = vi.fn();
const buildOrgBySlug = vi.fn();
const requireAdminOrgContext = vi.fn();
const getAdminFamilyRows = vi.fn();
...
vi.mock('@iconicedu/web/lib/admin/families', () => ({
  getAdminFamilyRows: (...args: unknown[]) => getAdminFamilyRows(...args),
}));

describe('GET /api/admin/families/list', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    getAdminFamilyRows.mockResolvedValue([]);
  });

  it('rejects non-admin users before loading family rows', async () => {
    ...
    const response = await GET(
      new Request(`${APP_URL}/api/admin/families/list?orgSlug=acme`),
    );
    ...
    expect(getAdminFamilyRows).not.toHaveBeenCalled();
  });
});
