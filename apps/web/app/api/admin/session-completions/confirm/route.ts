...
import { confirmSessionCompletionAsAdmin } from '@iconicedu/web/lib/api/session-completions';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await confirmSessionCompletionAsAdmin(supabase, {
      orgId,
      scheduleId,
      occurrenceKey,
    });
    ...
    return NextResponse.json(
      {
        success: false,
        message: error instanceof Error ? error.message : 'Unable to confirm session.',
      },
      { status: 500 },
    );
  }
