...
describe('GET /api/messages/channel-schedules', () => {
  it('returns 400 when channelId is missing', async () => {
    const response = await GET(
      new Request(`${APP_URL}/api/messages/channel-schedules`),
    );
    ...
    buildClassSchedulesByOrg.mockResolvedValueOnce([
      {
        ...
        title: 'Math Session',
        source: { kind: 'class_session', learningSpaceId: 'space-1', channelId: 'channel-1' },
      },
      {
        ids: { id: 'schedule-2', orgId: 'org-1' },
        title: 'Science Session',
        source: { kind: 'class_session', learningSpaceId: 'space-1', channelId: 'channel-2' },
      },
    ]);
