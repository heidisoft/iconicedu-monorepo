...
describe('POST /api/onboarding/student', () => {
  ...
  it('proxies the authenticated student request to apps/api', async () => {
    ...
    const response = await POST(
      new Request(`${APP_URL}/api/onboarding/student?org=i`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ inviteCode: 'ABC123' }),
      }),
    ...
    expect(fetchMock.mock.calls[0]).toEqual([
      'http://localhost:3001/onboarding/student?org=i',
      {
        method: 'POST',
        headers: {
