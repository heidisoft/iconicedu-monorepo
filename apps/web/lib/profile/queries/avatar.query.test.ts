...
describe('avatar.query', () => {
  it('creates signed and public URLs from the avatar bucket', async () => {
    ...
    const signed = await createSignedAvatarUrl(supabase, 'org-1/profile-1/avatar.jpg');
    const publicUrl = createPublicAvatarUrl(supabase, 'org-1/profile-1/avatar.jpg');

    expect(from).toHaveBeenCalledWith('public-avatars');
    expect(createSignedUrl).toHaveBeenCalledWith('org-1/profile-1/avatar.jpg', 3600);
    expect(getPublicUrl).toHaveBeenCalledWith('org-1/profile-1/avatar.jpg');
