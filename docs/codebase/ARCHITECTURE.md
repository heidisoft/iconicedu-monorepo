...
| Row → VM mapper layer                          | `apps/api/src/lib/mobile-data/message-mappers.ts`, `packages/shared-types/src/{rows,vm,payloads}`                                                      | Keeps DB shape (snake_case, nullable) separate from UI-ready shape (camelCase, non-null) per the documented type system    |
