...
/**
 * Class-session Join availability contract (issue #195).
 *
 * The API — not client-side role checks — decides whether an authenticated
 * effective actor may see and use a Join control for one exact class-session
 * occurrence.
 *
 * Rendering invariant: `eligible: true` means render an enabled Join control;
 * `eligible: false` means render no Join control at all. A permanently visible
 * disabled Join is not a valid state. Clients may disable the pressed control
 * transiently while a join request is in flight.
 */
export type ClassSessionJoinIneligibleReasonVM =
  | 'not_authorized'
...
export type ClassSessionJoinActionVM = 'join_integrated' | 'open_external';

/**
 * Identity of one exact occurrence. `occurrenceKey` is the *original* occurrence
 * start, matching `channel_live_sessions.occurrence_key` and the key used by
 * recurrence exceptions and overrides — not the rescheduled start.
 */
export interface ClassSessionOccurrenceIdentityVM {
  orgId: UUID;
  channelId: UUID;
  scheduleId: UUID;
...
export interface ClassSessionJoinAvailabilityVM {
  occurrence: ClassSessionOccurrenceIdentityVM;
  /** Start/end after recurrence overrides are applied. */
  effectiveStartAt: ISODateTime;
  effectiveEndAt: ISODateTime;
  eligible: boolean;
  action: ClassSessionJoinActionVM | null;
  provider: LiveSessionProviderVM | null;
  /** Stable machine-readable reason when `eligible` is false. */
  reason: ClassSessionJoinIneligibleReasonVM | null;
}

export interface ClassSessionJoinResultVM {
  sessionId: UUID;
  joinPath: string;
  status: LiveSessionAttendanceStatusVM;
  created: boolean;
