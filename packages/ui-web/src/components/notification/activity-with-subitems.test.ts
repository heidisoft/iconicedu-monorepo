...
import { groupHasUnreadSubActivities } from './activity-with-subitems';
...
describe('groupHasUnreadSubActivities', () => {
  it('returns true when any subactivity is unread', () => {
    expect(groupHasUnreadSubActivities([makeSubActivity(true), makeSubActivity(false)])).toBe(true);
  });

  it('returns false when all subactivities are read', () => {
    expect(groupHasUnreadSubActivities([makeSubActivity(true), makeSubActivity(true)])).toBe(
      false,
    );
  });
});
