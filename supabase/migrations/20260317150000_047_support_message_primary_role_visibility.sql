-- Ensure support visibility works when role data is on accounts.primary_role
-- and not materialized in user_roles rows.
          ...
          and exists (
            ...
            where a.org_id = m.org_id
              ...
              and (
                ur.id is not null
                or p.kind = 'staff'
                or a.primary_role in ('owner', 'admin', 'staff')
              )
          )
        )
      )
