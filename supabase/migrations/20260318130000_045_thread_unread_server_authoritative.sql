-- Deterministic per-user thread read-state + server-authoritative unread maintenance.
...
begin
  ...
  insert into public.thread_read_state (
    org_id,
    thread_id,
    channel_id,
    account_id,
  ...
  on conflict (org_id, thread_id, account_id)
  do update
...
for each row
execute function public.ensure_thread_read_state_for_participant();
...
begin
  if new.deleted_at is not null or new.thread_id is null or new.thread_parent_id is null then
    return new;
  end if;
  ...
  do update
    set channel_id = excluded.channel_id,
        unread_count = greatest(0, coalesce(public.thread_read_state.unread_count, 0)) + 1,
        updated_at = now(),
        deleted_at = null,
        deleted_by = null;
...
for each row
execute function public.increment_unread_for_thread_participants();
...
declare
  ...
  v_unread integer;
begin
  select trs.last_read_message_id, trs.last_read_at
    into v_effective_last_read_message_id, v_effective_last_read_at
    from public.thread_read_state trs
   where trs.org_id = p_org_id
     and trs.thread_id = p_thread_id
     and trs.account_id = p_account_id
...
grant execute on function public.recompute_unread_for_account_thread(
  uuid,
  uuid,
  uuid,
  uuid,
...
revoke all on function public.recompute_unread_for_account_thread(
  uuid,
  uuid,
  uuid,
  uuid,
...
select
  ...
  p.account_id,
  latest_reply.id,
  coalesce(latest_reply.created_at, now()),
  0,
  now(),
  now(),
  null,
...
left join lateral (
  select m.id, m.created_at
    from public.messages m
   where m.org_id = tp.org_id
     and m.thread_id = tp.thread_id
     and m.thread_parent_id is not null
     and m.deleted_at is null
   order by m.created_at desc
   limit 1
...
do update
  set channel_id = excluded.channel_id,
      last_read_message_id = coalesce(
        public.thread_read_state.last_read_message_id,
        excluded.last_read_message_id
      ),
      last_read_at = coalesce(public.thread_read_state.last_read_at, excluded.last_read_at),
      unread_count = coalesce(public.thread_read_state.unread_count, 0),
      deleted_at = null,
      deleted_by = null,
      updated_at = now();
